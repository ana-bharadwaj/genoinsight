...
E = input("Enter D (Is it deletion or duplication, or leave it empty):")
...
# Create two lists to separate matching ALT values ("<DEL>" and "<DUP>")
matched_data_alt_del = []
matched_data_alt_dup = []
...
for collection_name in collections:
    ...
    for document in collection.find():
        ...
        if Lower <= position < Upper and Lower < end <= Upper and C == chr:
            ...
            alt = extracted_data["ALT"]

            if E:
                if E == alt:
                    matched_data_alt_del.append(extracted_data)
                else: matched_data_alt_dup.append(extracted_data)
            else:
                alt_del_count = alt.count("<DEL>")
                alt_dup_count = alt.count("<DUP>")

                # Prioritize based on count of "<DEL>" or "<DUP>"
                if alt_del_count > alt_dup_count:
                    matched_data_alt_del.append(extracted_data)
                else:
                    matched_data_alt_dup.append(extracted_data)

            if collection_name not in matching_documents:
                total_match += 1

# Print matching documents with "<DEL>" count more first
for matched_doc in matched_data_alt_del:
...
# Print matching documents with "<DUP>" count more
for matched_doc in matched_data_alt_dup:
    print(f"Collection: {matched_doc['Collection']}, Document ID: {matched_doc['Document ID']}, Position: {matched_doc['Position']}, END: {matched_doc['END']}, ALT: {matched_doc['ALT']}")
